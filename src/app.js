  ...
  try {
    ...
    if (saved) {
      ...
      ['layout','series','episode','winTitle','cap1','cap2','handle'].forEach(function(key){
  ...
  function drawWindow(c, x, y, w, h, deg, panX, panY){
    ...
    if (state.winTitle) {
      ...
      c.fillStyle = 'rgba(255,255,255,.82)'; c.textBaseline = 'middle';
  ...
  function drawPill(c, x, cy, h){
    ...
    var tw = c.measureText(t).width, pad = h*0.42, w = tw + pad*2;
    c.shadowColor = 'rgba(0,0,0,.5)'; c.shadowBlur = h*0.3; c.shadowOffsetY = h*0.08;
    roundRect(c, x, cy - h/2, w, h, h/2); c.fillStyle = '#ffffff'; c.fill();
    noShadow(c);
    c.fillStyle = '#121316'; c.textBaseline = 'middle';
  ...
  // window's width. Taken from the reference layout: the phone overlaps the
  ...
  function render(c, p, opts){
    ...
    // logo row: the logo, then the series tag beside it
    var lh = 88*k, pillX = left;
    if (state.logo && ready.logo) {
      ...
      noShadow(c);
      pillX = left + lw + 0.022*W;
    }
    var hasRow = drawPill(c, pillX, top + lh/2, lh*0.56) > 0 || pillX > left;
    ...
    if (show) {
      ...
      if (portrait) {
        ...
        var capTop = l1 ? (px2 ? px2*1.06 : 0) + px1*0.74 : px2*0.74;
        ry1 = bottom - capTop - 0.035*H;
      } else {
        rx0 = cardsX; rx1 = right; ry0 = top; ry1 = bottom;
      }
      // Whichever arrangement comes out bigger in this room wins. 0.92 leaves
      // room for the corners the tilt swings outward.
      var lay = null, u = 0;
      (state.phone ? [SHOWCASE.side, SHOWCASE.stack] : [SHOWCASE.alone]).forEach(function(l){
        var fit = 0.92 * Math.min((rx1-rx0)/l.w, (ry1-ry0)/l.h);
        if (fit > u) { u = fit; lay = l; }
      });
      ...
      if (lay) {
        drawWindow(c, ox + lay.win.x*u, oy + lay.win.y*u, lay.win.w*u, lay.win.h*u, lay.win.deg,
                   state.panX*W, state.panY*H);
        if (state.phone) {
          var ph = lay.phone;
          drawPhone(c, ox + ph.x*u, oy + ph.y*u, ph.w*u, ph.h*u, ph.deg);
  ...
  function persist(){
    try{ localStorage.setItem('tf-state', JSON.stringify({
      ...
      layout:state.layout, series:state.series, episode:state.episode,
      winTitle:state.winTitle, phone:state.phone, cap1:state.cap1, cap2:state.cap2,
      handle:state.handle, dots:state.dots
    })); }catch(e){
